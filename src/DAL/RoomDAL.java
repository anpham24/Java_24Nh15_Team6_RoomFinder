...
public class RoomDAL {
    ...
    public List<RoomDTO> filterAdvanced(String keyword, double minPrice, double maxPrice,
        ...
        List<RoomDTO> list = new ArrayList<>();
        StringBuilder sb = new StringBuilder("SELECT DISTINCT r.* FROM rooms r ");
        ...
        if (amenityIds != null && !amenityIds.isEmpty()) {
            ...
            for (int i = 0; i < amenityIds.size(); i++) sb.append(i == 0 ? "?" : ",?");
            sb.append(") ");
        }
        sb.append(sortByPrice ? "ORDER BY r.price ASC" : "ORDER BY r.created_at DESC");
